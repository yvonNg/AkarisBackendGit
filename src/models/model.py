...
class CropDtl(Base):
    ...
    plantation_date = Column(DateTime, nullable=False)
...
class PlantMethod(Base):
    ...
    method = Column(SqlEnum(PlantMethodEnum), nullable=False, index=True)
    other_method = Column(String(100), nullable=True)
    record_created_by = Column(Integer, ForeignKey("users.user_id"), nullable=False, index=True)
